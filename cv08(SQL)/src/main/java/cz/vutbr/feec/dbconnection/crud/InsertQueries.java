...
public class InsertQueries {
  ...
  public void insertNewUser(String email, String name, String surname) {
    ...
    String insertUser = ("INSERT INTO user " + "(email,name,surname)"
            + "VALUES(?, ?,?)");
  ...
  public void insertRole(String role) {
	  if (role==null) {
		  throw new NullPointerException("Role must be set");
	  }
	  Connection conn = DBConnection.getDBConnection();
	  ...
	  String insertRole = "insert into role (name) values(?)";
			  ...
			  try {
		  PreparedStatement prStmt = conn.prepareStatement(insertRole);
